...
if plotFlag == true        
    ...
    subplot(5,1,3);yyaxis left; plot(in.trials(currentTrial).touchFilt); title('Touch Capacitance w/ baseline (Blue) and Difference w/ threshold (Orange), this trial')
    hold on; plot(in.trials(currentTrial).touchBaseline); 
    yyaxis right; plot(in.trials(currentTrial).touchBaseline-in.trials(currentTrial).touchFilt); % difference bet. baseline and touchFilt
    dThresh=15  ; plot(dThresh*ones(1,length(in.trials(currentTrial).touchBaseline)),'LineStyle','--');
    ...
    subplot(5,1,1);
    H = histogram(allFailFlags,[-0.5:1:18.5]); title('All fail codes from entire session, current fail code group highlighted')
