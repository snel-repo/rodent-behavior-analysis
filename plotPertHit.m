function plotPertHit(extractData, time_of_interest)
for i = 1 : length(extractData.posHitPert) % plot all hit trials
    ...
    subplot(2,2,2)
    q = plot([0:extractData.sampleTime:(extractData.sampleTime*length(extractData.velHitPert{i})-extractData.sampleTime)], extractData.velHitPert{i}, 'g');
    q.Color(4) = 0.4;
    xlim([0 time_of_interest])
    ...
    if length(extractData.velHitPert{i}) > 1
        q = plot([0:extractData.sampleTime:(extractData.sampleTime*length(extractData.velHitPert{i})-2*extractData.sampleTime)],  diff(extractData.velHitPert{i}) / (extractData.sampleTime * 1e-3), 'g');
        q.Color(4) = 0.4;
        xlim([0 time_of_interest])
