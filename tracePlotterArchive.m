function [attemptTrial] = tracePlotter(trials, dataPrior)
close all
...
for i = 1:length(trials)
    ...
    for j = 1:min(length(zeroStartIdx), length(zeroEndIdx))
        if hitIdx > zeroStartIdx(j) && hitIdx <= zeroEndIdx(j) && trials(i).hitTrial % if the attempt is a hit
            ...
            successAttempts(totalHitTurns).minPos = min(trials(i).pos(zeroStartIdx(j) : zeroEndIdx(j)));
            successAttempts(totalHitTurns).minVel = min(trials(i).vel(zeroStartIdx(j) : zeroEndIdx(j)));
    ...
    if attemptTotalTrial ~= 0
        if attemptTotalTrial == 1 && trials(i).hitTrial
            attemptTrial(i).firstAttemptSuccess = 1;
        else
            attemptTrial(i).firstAttemptSuccess = 0;
        end
        attemptTrial(i).totalAttempts = attemptTotalTrial;
    else
        attemptTrial(i).totalAttempts = NaN;
        attemptTrial(i).firstAttemptSuccess = NaN;
    end
    
    if trials(i).bidirectionalEnable && attemptTrial(i).firstAttemptSuccess == 1 && trials(i).dirTask == 0 && trials(i).hitTrial
        attemptTrial(i).labelAttempt = 1; % 1 is for CW turns succeeded on first attempt
    elseif trials(i).bidirectionalEnable && attemptTrial(i).firstAttemptSuccess == 1 && trials(i).dirTask == 1 && trials(i).hitTrial
        attemptTrial(i).labelAttempt = 2; % 2 is for CCW turns succeeded on first attempt
    elseif trials(i).bidirectionalEnable == 0
        attemptTrial(i).labelAttempt = NaN; % 0 is for trials that did not succeed on the first attempt, or did not have any attempts.
    elseif trials(i).bidirectionalEnable && attemptTrial(i).totalAttempts > 1 || isnan(attemptTrial(i).totalAttempts)
        attemptTrial(i).labelAttempt = 0;
    end
end
...
totalHitRate = sum([trials(:).wasHit]) / (length(trials) - nonMoveTrial);
